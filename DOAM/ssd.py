	...
	for v in cfg:
		if v == 'M':
			...
			if batch_norm:
				...
				# layers += [BasicConvBN(in_channels, v, kernel_size=3, padding=1)]
			else:
...
class BasicConvBN(nn.Module):
	...
	def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1, groups=1, relu=True,
		...
		self.conv = nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding,
													dilation=dilation, groups=groups, bias=bias)
		self.bn = nn.BatchNorm2d(out_planes) if bn else None  # ,eps=1e-5, momentum=0.01, affine=True
		self.relu = nn.ReLU(inplace=True) if relu else None
...
def build_ssd(phase, size=300, num_classes=21, mode=None, type="ssd"):
	...
	return SSD(phase, size, base_, extras_, head_, num_classes, mode, type=type, ft_module=layers,
